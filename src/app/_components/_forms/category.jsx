...
const NewCategory = ({
  ...
  const submitNewCategory = async () => {
    ...
    try {
      ...
      setMessage(response.data.newCategory.message)
      refetch()
  ...
  const submitUpdateCategory = async () => {
    ...
    try {
      ...
      refetch()
      setLoading('')
      setMessage(response.data.updateCategory.message)
          ...
          <div className="relative bg-white rounded-xl shadow-xl dark:bg-gray-700">
            <div className="flex items-center justify-between pt-5 pb-3 px-5 rounded-lg dark:border-gray-600 h-[50px]">
                <h3 className="flex flex-col">
                  <span className="font-poppins text-[24px] font-[900] text-gray-900 dark:text-white">{edit ? 'Update Category' : 'Create Category'}</span>
                </h3>
                {category && checkObjectValues(category, images) && !edit &&
                  <div 
                ...
                {category && checkObjectValues(category, images) && edit == 'category' &&
                  <div 
                    className="w-min flex justify-center ml-3"
            ...
            <div className="flex items-center justify-between p-2 md:p-5 border-b rounded-lg dark:border-gray-600 h-[20px]">
                <h3 className="flex flex-col">
                  <span 
                    ...
                    onClick={() => (dispatch(changeView('categories'), dispatch(changePopup(''))))}
                  >
                    view all
                  </span>
                </h3>
            ...
            <div className="p-4 md:p-5 space-y-4 h-[40rem] overflow-y-scroll">
              <InputField 
                ...
                property={'name'}
                dispatch={dispatch}
                stateMethod={changeCategoryValue}
                id="name"
              />
            </div>
          </div>
